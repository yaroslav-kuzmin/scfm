...
#define DEFAULT_TIMEOUT_CONNECT        500000
...
static flag_t read_state_controller(link_s * link,controller_s * controller)
{
	flag_t rc = SUCCESS;
	state_controller_s state;
	control_controller_s * control = controller->control;
...
static flag_t read_config_controller(link_s * link,controller_s * controller)
{
	flag_t rc;
	...
	rc = controller_check_config(controller->config,&config);
	if(rc == FAILURE){
		g_mutex_lock(control->mutex);
		controller->status_config = NOT_OK;
		g_mutex_unlock(control->mutex);
	}
	else{
		g_mutex_lock(control->mutex);
		controller->status_config = OK;
		g_mutex_unlock(control->mutex);
	}

	return SUCCESS;
}
static flag_t write_command_controller(link_s * link,controller_s * controller)
{
	flag_t rc = SUCCESS;
	command_u command;
	control_controller_s * control = controller->control;
	...
	g_mutex_lock(control->mutex);
	command.all = controller->command.all;
	controller->command.part.value = COMMAND_EMPTY;
	g_mutex_unlock(control->mutex);
	if(command.part.value != COMMAND_EMPTY){
		rc = device_write_command(link,command);
		if(rc == FAILURE){
			return rc;
		}
	}
...
static flag_t read_controller(link_s * link,controller_s * controller)
{
	flag_t rc = SUCCESS;
...
static flag_t read_write_controller(link_s * link,controller_s * controller)
{
	flag_t rc = SUCCESS;
	rc = read_state_controller(link,controller);
	if(rc == FAILURE){
	...
	rc = write_command_controller(link,controller);
	if(rc == FAILURE){
		return rc;
	}
	return rc;
}
static flag_t check_timeout_controller(controller_s * controller,uint32_t timeout_connect)
{
	flag_t rc = FAILURE;
	control_controller_s * control = controller->control;
	uint32_t timeout;
	...
	if(control->counter > timeout_connect){
		control->counter -= timeout_connect;
	}
	else{
		control->counter = timeout;
		rc = SUCCESS;
	}
	return rc;
}
...
/*****************************************************************************/
static flag_t connect_controllers(connect_s * connect)
{
	GSList * list = connect->list_controllers;
	GThread * thread;
...
static flag_t control_controllers_on(block_controller_s * bc)
{
	...
	if( list == NULL){
		g_info("В базе данных нет контролеров!");
		return FAILURE;
	}
...
{
	block_controller.list_connect = NULL;
	return SUCCESS;
}
...
{
	GSList * list = block_controller.list_connect;
	for(;list;){
		connect_s * connect = list->data;
		del_connect(connect);
		list = g_slist_next(list);
	}
	list = block_controller.list_connect;
	g_slist_free(list);
	return SUCCESS;
}
...
/*****************************************************************************/
static flag_t check_link(link_s * connect,link_s * controller)
{
	flag_t rc;

	if(connect->type != controller->type){
		return FAILURE;
	}

	if(connect->type == TYPE_LINK_UART){
		rc = g_strcmp0(connect->device,controller->device);
		if(rc != 0){
			return FAILURE;
		}
	}
		...
		if(connect->type == TYPE_LINK_TCP){
			rc = g_strcmp0(connect->address,controller->address);
			if(rc != 0){
				return FAILURE;
			}
			if(connect->port != controller->port){
				return FAILURE;
			}
		}
		else{
...
static flag_t copy_link(link_s * connect,link_s * controller)
{
	connect->type = controller->type;
	if(connect->type == TYPE_LINK_UART){
			connect->device = g_strdup(controller->device);
			connect->baud = controller->baud;
			connect->parity = controller->parity;
			connect->data_bit = controller->data_bit;
			connect->stop_bit = controller->stop_bit;
	}
	else{
		if(connect->type == TYPE_LINK_TCP){
				connect->address = g_strdup(controller->address);
...
static connect_s * new_connect(controller_s * controller)
{
	connect_s * connect;

	connect = g_slice_alloc0(sizeof(connect_s));
	connect->link = g_slice_alloc0(sizeof(link_s));
	copy_link(connect->link,controller->link);
	connect->status = STATUS_OFF;
	connect->timeout = 0;
	g_mutex_init(&(connect->mutex));
	if(connect->link->type == TYPE_LINK_UART){
		g_string_printf(pub,"file : %s",connect->link->device);
		connect->name = g_strdup(pub->str);
	}
	else{
		if(connect->link->type == TYPE_LINK_TCP){
...
static flag_t del_connect(connect_s * connect)
{
	...
	g_slist_free(connect->list_controllers);
	g_mutex_clear(&(connect->mutex));
	g_slice_free1(sizeof(connect_s),connect);

	return SUCCESS;
}

static flag_t add_connect(block_controller_s * bc,controller_s * controller)
{
	flag_t rc;
	GSList * list = bc->list_connect;
	connect_s * connect = NULL;

	/*проверка на наличие одинакового подключения*/
	for(;list;){
		connect = list->data;
		rc = check_link(connect->link,controller->link);
		if(rc == SUCCESS){
			break;
		}
		list = g_slist_next(list);
	...
	if(list == NULL){
		connect = new_connect(controller);
		list = g_slist_append(list,connect);
		bc->list_connect = list;
	}

	connect->list_controllers = g_slist_append(connect->list_controllers,controller);
	controller->control->mutex = &(connect->mutex);
	return SUCCESS;
}
...
controller_s * init_controller(object_s * object)
{
	...
	controller->status_link = STATUS_OFF;
	...
	if(rc != SUCCESS){
		...
		g_slice_free1(sizeof(GMutex),controller->control->mutex);
	...
	add_connect(&block_controller,controller);
...
int del_property_controller(controller_s * property)
{
	...
	g_slice_free1(sizeof(GMutex),control->mutex);
